...
local function factory(args)
	...
	local function update_synced()
		...
		pcall(function()
			mounts_get = GioUnix.unix_mounts_get
			mount_get_mount_path = GioUnix.unix_mount_get_mount_path
		end)
		...
		local header_fmt = "%-" .. tostring(pathlen) .. "s  %5s  %8s  %8s"
		local notifytable = {
			[1] = "<b>" .. string.format(header_fmt, "PATH", "USED", "FREE", "SIZE") .. "</b>",
		}
		local fmt = "\n%-" .. tostring(pathlen) .. "s  %4s%%  %8.2f  %8.2f %s"
